...
export default function FormPreview({ fields, successMessage }: Props) {
  ...
  const hasError = (n: string) =>
    Object.prototype.hasOwnProperty.call(errors, n);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid" style={{ rowGap: 12 }}>
      {fields.map((field) => {
      ...
      <div style={{ gridColumn: "1 / -1", display: "flex", gap: 8, justifyContent: "flex-end" }}>
        <button type="button" onClick={() => { reset(); setSubmitted(false); }}
          className="btn" style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid var(--border)", background: "#151a2e", color: "var(--text)" }}>
          Reset
        </button>
        <button type="submit" className="btn"
          style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #3b4ea3", background: "#2a3d85", color: "var(--text)" }}>
          Submit
        </button>
      </div>

      {submitted && (
        <div style={{ gridColumn: "1 / -1", marginTop: 8, padding: 10, borderRadius: 10, border: "1px solid #1f7a4a", background: "#0f2d1f", color: "#d9ffea" }}>
